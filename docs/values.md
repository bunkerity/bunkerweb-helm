...
| `bunkerweb.minReadySeconds`                             | Seconds a freshly started pod must stay ready before the DaemonSet rollout moves on to the next node... | `int`    | `0`                                                     |
...
| `bunkerweb.updateStrategy`                              | Rollout strategy (DaemonSet kind only, ignored by Deployment & StatefulSet) type: "RollingUpdate" (d... | `object` | See nested values                                       |
...
| `bunkerweb.updateStrategy.rollingUpdate`                | Optional maxSurge (integer or percentage) requires hostPorts: false                                     | `object` | See nested values                                       |
| `bunkerweb.updateStrategy.type`                         | Configuration for type                                                                                  | `string` | `"RollingUpdate"`                                       |
...
| `bunkerweb.updateStrategy.rollingUpdate.maxUnavailable` | Integer or percentage of nodes allowed unavailable during a rollout Keep 1 to replace a single node ... | `int`    | `1`                                                     |
...
| `networkPolicy.ingress.apiPorts`          | The BunkerWeb internal API, admitted from the callers above. One rule covers the whole surface: /con... | `list`   | `[5000, 5443]`    |
| `networkPolicy.ingress.extraRules`        | Extra ingress rules appended verbatim, for any other listener you enable on the BunkerWeb pods. A st... | `list`   | `[]`              |
...
| `service.loadBalancerIP`        | Set loadBalancerIP (if type is LoadBalancer) Allows binding to a floating IP                            | `string` | `""`              |

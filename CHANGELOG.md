...
- [FEATURE] `networkPolicy.ingress`: the policy is no longer egress only. When `networkPolicy.enabled` is true it now also restricts ingress to the worker pods: `allowWeb` keeps 8080/8443 open, `apiPort` (5000) is admitted only from the scheduler, Web UI and API pods, and `allowMetrics` admits the metrics port from this chart's Prometheus pods plus anything in `metricsFrom`, or from everywhere with `metricsAllowAll`. With no peer to admit the metrics rule is omitted rather than rendered with an empty `from`, which a NetworkPolicy reads as every source. Enforcement needs a CNI that implements NetworkPolicy; the token is the layer that holds regardless.
